...
def make_cbz(
    input_dir, output_path="", output_dir="", output_base_name="", save_as_ext=".cbz"
):
    if not output_path:
        output_path = os.path.join(
            output_dir, f"{output_base_name}_translated{save_as_ext}"
        )
        ...
        for root, dirs, files in os.walk(input_dir):
            for file in files:
                if is_image_file(file):
                    ...
                    archive.write(
                        file_path, arcname=os.path.relpath(file_path, input_dir)
                    )
...
def make_pdf(input_dir, output_path="", output_dir="", output_base_name=""):
    ...
    sorted_paths = sorted(
        image_paths, key=lambda x: (get_number(x) is None, get_number(x))
    )
...
def make(input_dir, output_path="", save_as_ext="", output_dir="", output_base_name=""):
    if not output_path and (not output_dir or not output_base_name):
        raise ValueError(
            "Either output_path or both output_dir and output_base_name must be provided"
        )
