...
def lists_to_blk_list(
    ...
    for blk in blk_list:
        ...
        sorted_entries = sort_textblock_rectangles(
            blk_entries, blk.source_lang_direction
        )
...
def validate_ocr(main_page, source_lang):
    ...
    if ocr_tool == tr("Microsoft OCR") and not credentials.get(
        tr("Microsoft Azure"), {}
    ).get("api_key_ocr"):
        # Messages.show_api_key_ocr_error(main_page)
        return False

    if ocr_tool == tr("Google Cloud Vision") and not credentials.get(
        tr("Google Cloud"), {}
    ).get("api_key"):
        # Messages.show_api_key_ocr_error(main_page)
        return False
    ...
    # Validate GPT OCR
    if ocr_tool == tr("GPT-4.1-mini") and not credentials.get(
        tr("Open AI GPT"), {}
    ).get("api_key"):
        # Messages.show_api_key_ocr_error(main_page)
        return False
    ...
    if source_lang_en == "Russian":
        if ocr_tool == tr("Default") and not credentials.get(tr("Open AI GPT"), {}).get(
            "api_key"
        ):
...
def validate_translator(main_page, source_lang, target_lang):
    ...
    if translator_tool == tr("DeepL") and not credentials.get(tr("DeepL"), {}).get(
        "api_key"
    ):
        # Messages.show_api_key_translator_error(main_page)
        return False

    if translator_tool == tr("Microsoft Translator") and not credentials.get(
        tr("Microsoft Azure"), {}
    ).get("api_key_translator"):
        # Messages.show_api_key_translator_error(main_page)
        return False

    if translator_tool == tr("Yandex") and not credentials.get(tr("Yandex"), {}).get(
        "api_key"
    ):
        # Messages.show_api_key_translator_error(main_page)
        return False

    if "GPT" in translator_tool and not credentials.get(tr("Open AI GPT"), {}).get(
        "api_key"
    ):
        # Messages.show_api_key_translator_error(main_page)
        return False

    if "Gemini" in translator_tool and not credentials.get(tr("Google Gemini"), {}).get(
        "api_key"
    ):
        # Messages.show_api_key_translator_error(main_page)
        return False

    if "Claude" in translator_tool and not credentials.get(
        tr("Anthropic Claude"), {}
    ).get("api_key"):
        # Messages.show_api_key_translator_error(main_page)
        return False
    ...
    if translator_tool == tr("Google Translate"):
        if source_lang == main_page.tr(
            "Brazilian Portuguese"
        ) or target_lang == main_page.tr("Brazilian Portuguese"):
